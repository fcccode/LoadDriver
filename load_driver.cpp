...
{
	NTSTATUS nStatus;
	PSYSTEM_GDI_DRIVER_INFORMATION pgdi_info=NULL;

	pgdi_info=(PSYSTEM_GDI_DRIVER_INFORMATION)ExAllocatePool(NonPagedPool,sizeof(SYSTEM_GDI_DRIVER_INFORMATION));
	RtlZeroMemory(pgdi_info,sizeof(SYSTEM_GDI_DRIVER_INFORMATION));

	RtlInitUnicodeString(&pgdi_info->DriverName,L"\\??\\C:\\gdi.sys");
	
	nStatus=ZwSetSystemInformation(SystemLoadGdiDriverInSystemSpace,pgdi_info,sizeof(SYSTEM_GDI_DRIVER_INFORMATION));
	if(NT_SUCCESS(nStatus))
		DbgPrint("\nImage loaded");
	else
		DbgPrint("\Image not Loaded");
